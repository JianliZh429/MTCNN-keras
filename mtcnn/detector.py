...
class Detector:
    ...
    def predict(self, np_images, verbose=False):
        ...
        for im in np_images:
            ...
            if self.r_net is not None:
                ...
                if boxes_c is None:
                    logger.info('Bounding boxes is None, after RNet')
                    all_boxes.append(np.array([]))
                    landmarks.append(np.array([]))
                    continue
            if self.o_net is not None:
                ...
                if boxes_c is None:
                    logger.info('Bounding boxes is None, after ONet')
                    all_boxes.append(np.array([]))
                    landmarks.append(np.array([]))
                    continue
    ...
    def predict_with_p_net(self, im):
        ...
        while min_len >= 12:
            ...
            min_len = min_len * self.scale_factor
            factor_count += 1
        ...
        for scale in scales:
            ...
            boxes = generate_bbox(labels[:, :, 1], bboxes, scale, self.threshold[0])
